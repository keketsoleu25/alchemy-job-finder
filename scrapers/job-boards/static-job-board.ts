...
function externalId(url: string): string | undefined {
  const cj = url.match(/(?:job|jobs)[^0-9]*(\d{6,})/i)?.[1];
  if (cj) return cj;
  const pnet = url.match(/\/job\/[^/?#]*?(\d{5,})(?:[/?#]|$)/i)?.[1];
  if (pnet) return pnet;
  const careers24 = url.match(/(?:job|vacancy)[^0-9]*(\d{4,})/i)?.[1];
  return careers24;
}
...
function looksLikeJobLink(board: NonNullable<ReturnType<typeof boardName>>, href: string): boolean {
  ...
  if (board === "PNet") return /\/job\//i.test(href);
...
function listingLinks(html: string, base: string, board: NonNullable<ReturnType<typeof boardName>>): string[] {
  ...
  for (const match of html.matchAll(anchor)) {
    ...
    try { links.add(absoluteUrl(href, base)); } catch { /* ignore malformed public links */ }
...
function jsonLdJobs(html: string): Record<string, unknown>[] {
  ...
  for (const match of html.matchAll(scripts)) {
    ...
    } catch { /* malformed JSON-LD should not kill a board run */ }
  }
  return jobs;
}

function text(value: unknown): string | undefined {
  return typeof value === "string" && value.trim() ? decode(value) : undefined;
}
...
function fromLd(value: Record<string, unknown>, pageUrl: string, fallbackCompany: string): ScrapedJob | null {
  ...
  const date = text(value.datePosted);
  const publishedAt = date && !Number.isNaN(Date.parse(date)) ? new Date(date) : undefined;
  ...
  const remote = /remote|telecommute|work from home/i.test(`${location ?? ""} ${description} ${String(value.jobLocationType ?? "")}`);
  return { externalId: id ?? externalId(pageUrl), title, company, location, remote, employmentType: employment, description, applyUrl, sourceUrl: pageUrl, source: "COMPANY_SITE", publishedAt, scrapedAt: new Date() };
}

function fallbackFromHtml(html: string, pageUrl: string, fallbackCompany: string): ScrapedJob | null {
  const h1 = html.match(/<h1\b[^>]*>([\s\S]*?)<\/h1>/i)?.[1];
  const titleTag = html.match(/<title\b[^>]*>([\s\S]*?)<\/title>/i)?.[1];
  const title = decode(h1 ?? titleTag ?? "").replace(/\s*[|–-]\s*(PNet|CareerJunction|Careers24).*$/i, "").trim();
  if (!title) return null;
  const main = html.match(/<main\b[^>]*>([\s\S]*?)<\/main>/i)?.[1] ?? html.match(/<body\b[^>]*>([\s\S]*?)<\/body>/i)?.[1] ?? "";
  const description = decode(main).slice(0, 50_000);
...
export class StaticJobBoardScraper implements JobScraper {
  async fetch(company: ScraperCompanyConfig): Promise<ScrapedJob[]> {
    const board = boardName(company.careerUrl);
    if (!board) throw new Error(`Unsupported job board URL: ${company.careerUrl}`);
    ...
    if (!links.length) throw new Error(`${board}: no public vacancy links found`);
    const batches = await mapLimit(links, 4, async (url) => {
      try {
        const html = await fetchHtml(url);
        const ld = jsonLdJobs(html).map((job) => fromLd(job, url, company.name)).find(Boolean);
        return ld ?? fallbackFromHtml(html, url, company.name);
      } catch (error) {
        console.warn(`  ${board}: detail skipped ${url}: ${error instanceof Error ? error.message : String(error)}`);
        return null;
      }
    });
    const jobs = batches.filter((job): job is ScrapedJob => Boolean(job));
    if (!jobs.length) throw new Error(`${board}: vacancy pages produced no parseable jobs`);
    return jobs;
  }
}
